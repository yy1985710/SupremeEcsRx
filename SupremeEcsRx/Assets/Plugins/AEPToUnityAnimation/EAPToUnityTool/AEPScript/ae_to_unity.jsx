{
			...
			if (this.showInitialDialog()) {
				...
				for (var i=0; i<numSelected; i++) {
					...
					if (rootItem instanceof CompItem) {
						...
						this.progressWindow = new Window("palette","EAPToUnity Animation",undefined,{"closeButton":false});
						this.progressBar = this.progressWindow.add("progressbar",undefined,0,this.numLayers);
						this.progressWindow.show();
						var aepData = this.processCompositions();
						this.aepData.push( aepData );
					}
				}

				this.jsonData = this.combineAepData( this.aepData );
	...
	AE2JSON.prototype.combineAepData = function(aepData){
		var numEAPs = aepData.length;
		if (numEAPs > 1) {
			var firstAnimName = this.getAnimationName(aepData[0]);
			this.addAnimationNameToSkin(aepData[0]["skins"]["default"],firstAnimName);
			for (var i=1; i<numEAPs; i++) {
				var boneData = aepData[i]["bones"];
				aepData[0]["bones"] = this.mergeByName( aepData[0]["bones"], aepData[i]["bones"] );
				aepData[0]["slots"] = this.mergeByName( aepData[0]["slots"], aepData[i]["slots"] );
				var animName = this.getAnimationName(aepData[i]);
				this.addAnimationNameToSkin( aepData[i]["skins"]["default"], animName );
				this.mergeSkins( aepData[0]["skins"]["default"], firstAnimName,
					aepData[i]["skins"]["default"], animName);
				aepData[0]["animations"][animName] = aepData[i]["animations"][animName];
			}
			var animations = aepData[0]["animations"];
			var skins = aepData[0]["skins"]["default"];
			for (var slotName in skins) {
				for (var texName in skins[slotName]) {
					...
					for (var animName in animations) {
						...
						var slotData = this.findByName(aepData[0]["slots"],slotName);
	...
	AE2JSON.prototype.getAnimationName = function(aepData){
		var name;
		for (name in aepData["animations"]) {
			break;
		}
		return name;
	}
	...
	AE2JSON.prototype.showInitialDialog = function(){
		var w = new Window("dialog","Export Data To Unity",undefined,{"closeButton":false});
		var group, label;
		group = w.add("group");
		
		label = group.add("statictext",undefined,"Animation Threshold ");
		...
		checkbox = group.add("checkbox { text:'Smooth Rotation', value: false }");
		
		
		group = w.add("group");
		
		//label = group.add("statictext",undefined,"Filename Suffix:");
		//var suffix = group.add("edittext",undefined,"_unity");
		...
		isSmooth=checkbox.value;
		this.spatialTolerance = (slider.value == slider.maxValue) ? 999 : ((slider.value/slider.maxValue)/10.0);
		//this.filenameSuffix = suffix.text;
		return true;
	}

	AE2JSON.prototype.progress = function(title){
		var progress = ++this.progressBar.value;
		this.progressWindow.close();
		this.progressWindow = new Window("palette",title,undefined,{"closeButton":false});
		this.progressBar = this.progressWindow.add("progressbar",undefined,progress,this.numLayers);
		this.progressWindow.show();
	}

	AE2JSON.prototype.progressFinish = function(title){
		this.progressWindow.close();
		this.progressWindow = new Window("palette",title,undefined,{"closeButton":false});
		this.progressBar = this.progressWindow.add("progressbar",undefined,100,100);
		this.progressWindow.show();
	}

	AE2JSON.prototype.progressInit1 = function(title,maxRef){
		var progress = ++this.progressBar.value;
		this.progressWindow.close();
		this.progressWindow = new Window("palette",title,undefined,{"closeButton":false});
		this.progressBar = this.progressWindow.add("progressbar",undefined,progress,maxRef);
		this.progressWindow.show();
	}

	AE2JSON.prototype.resetProgress = function(){
		this.progressWindow = new Window("palette","process...",undefined,{"closeButton":false});
		this.progressBar = this.progressWindow.add("progressbar",undefined,0,this.numLayers);
		this.progressWindow.show();
	}
	...
	AE2JSON.prototype.showCompleteDialog = function(){
		var w = new Window("dialog","Export Data To Unity",undefined,{"closeButton":false});
		w.add("statictext",undefined,"Success!");
		w.add("statictext",undefined,"File Export: "+this.FilenameCreate);
		this.computeStats();
		w.add("statictext",undefined,"Total Bones: "+this.totalBones);
		w.add("statictext",undefined,"Total Frames: "+this.totalKeyframes);
	...
	AE2JSON.prototype.initializeCompositions = function(rootItem){
		...
		this.resetProgress();
		var maxRef=this.referencedComps.length;
		while (i < this.referencedComps.length) {
			this.progressInit1("Collect Composition...",100);
	...
	AE2JSON.prototype.processCompositions = function(){
		for (var compName in this.compData) {
			...
			this.compData[compName] = this.generateAepData();
		}
	...
	AE2JSON.prototype.getEAPBlendMode = function(blendingMode){
		var result = "normal";
		switch (blendingMode) {
	...
	AE2JSON.prototype.combineCompositions = function(){
		...
		var countLimit=0;
		this.resetProgress();
		//alert("combineCompositions");
		while (i < this.jsonData.bones.length) {
			this.progress("Combine Composition...");
			if (this.jsonData.bones[i].hasOwnProperty("comp")) 
			{
				
				this.progress("Please wait, maybe too long...");
				var bone = this.jsonData.bones[i];
		...
		this.progressFinish("Combine Composition...");
		//alert("combineCompositions finish");
	}

	AE2JSON.prototype.cloneObj= function(obj) {
	...
	AE2JSON.prototype.copyCompData = function(parentBoneName,compName,compData,compInPoint,compAnchorPoint,compAnimation,
		...
		compData = this.cloneObj(compData);
		//
		// Copy bones
		//
		...
		for (var i=0; i<numSlots; i++) {
			...
			var blend = this.getEAPBlendMode(compBlendingMode);
			if (blend != "normal") {
...
// alert(JSON.stringify(animEntry["color"],null,"\t"));
				...
				if(!myLayer.adjustmentLayer == true){
					...
					switch(layerType){
						case "NULL":
						 	...
						 	if(isSmooth)
							{
								layerDataSmooth.push(new Null(this.jsonData.composition.compSettings, myLayer, 0));
							}
							break;
						case "AV":
							...
							if(isSmooth)
							{
								smooth = new AV(this.jsonData.composition.compSettings, myLayer, 0);
								if (smooth.source instanceof CompItem) {
									this.referencedComps.push(myLayer.source)
								}
								layerDataSmooth.push(smooth);
							}
							break;
						case "SHAPE":
							...
							if(isSmooth)
							{
								layerDataSmooth.push(new ShapeObj(this.jsonData.composition.compSettings, myLayer, 0));
							}
							break;
						default:
	...
	AE2JSON.prototype.makeEAPBoneName = function(layer) {
	...
	AE2JSON.prototype.makeEAPSlotName = function(layer) {
		return this.makeEAPBoneName(layer);
	}


	AE2JSON.prototype.makeEAPAttachmentNameStr = function(sourceName) {
		if (sourceName == null) {
			return null;
		} else {
	...
	AE2JSON.prototype.makeEAPAttachmentName = function(layer, time) {
		...
		return this.makeEAPAttachmentNameStr( name );
	}

	AE2JSON.prototype.makeEAPAttachmentNames = function(layer) {
		var result = new Array()
		if (!layer.files) {
			result.push( this.makeEAPAttachmentName(layer) );
		} else {
			var numFiles = layer.files.length;
			for (var i=0; i<numFiles; ++i) {
				//alert( layer.files[i] );
				result.push( this.makeEAPAttachmentNameStr( layer.files[i] ));
			}
		}
		return result;
	}
	...
	AE2JSON.prototype.ShowDebug=function(toFile,jsonString)
	{
	...
	AE2JSON.prototype.generateEAPBones = function() {
		...
		while( boneGenerated.length <= numLayers ) {
			...
			for (var i=0; i<numLayers; i++) {
				...
				var layer = layers[i];
				//var jsonString = JSON.stringify(layer, null, "\t");
				//this.ShowDebug(true,jsonString);
				//return;
				var boneName = this.makeEAPBoneName( layer );
				...
				if ((layer.layerType == "AV" || layer.layerType == "Shape") && parentExists &&
					...
					//var ux = layer.transform.scale_timeline[0][2][0] ;
					//var uy = layer.transform.scale_timeline[0][2][1] ;
					...
					if (parentIndex > 0) {
						...
						parentName = this.makeEAPBoneName( parent );
					...
					if (layer.comp) {
						...
						boneData["px"]= layer.transform.anchorPoint_timeline[0][2][0];
						boneData["py"]= layer.transform.anchorPoint_timeline[0][2][1];
	...
	AE2JSON.prototype.generateEAPSlots = function() {
		var slotsData = [];
		var layers = this.jsonData.composition.layers;
		var numLayers = layers.length;
		for (var i=numLayers-1; i>=0; i--) {
			var layer = layers[i];
			if (layer.enabled) {
				var slotName = this.makeEAPSlotName( layer );
				var found = false;
				...
				if (!found) {
					var attachmentName = this.makeEAPAttachmentName( layer, 0 );
					var slotData = {
						"name": slotName,
						"bone": this.makeEAPBoneName( layer )
					};
					if (layer.comp) {
					...
					var blend = this.getEAPBlendMode(layer.blendingMode);
					if (blend != "normal") {
	...
	AE2JSON.prototype.generateEAPSkins = function() {
		var skinsData = {};
		var layers = this.jsonData.composition.layers;
		var numLayers = layers.length;
		for (var i=numLayers-1; i>=0; i--) {
			...
			if (!layer.comp && layer.enabled) {
				var slotName = this.makeEAPSlotName( layer );
				var attachmentNames = this.makeEAPAttachmentNames( layer );
	...
	AE2JSON.prototype.generateEAPSlotAnimations = function() {
		var slotAnimData = {};
		var layers = this.jsonData.composition.layers;
		var numLayers = layers.length;
		var numKeys, time;
		...
		for (var i=numLayers-1; i>=0; i--) {
			...
			if (layer.inPoint > 0.0) {
				...
				if (!attachmentTimeline) {
					...
					attachmentName = this.makeEAPAttachmentName( layer, layer.inPoint );
				...
				if (layer.timeRemap) {
					for (var index=0; index<numKeys; index++) {
						...
						for (var j=0; j<numFrames; j++) {
							...
							var thisFramesAttachmentName = this.makeEAPAttachmentNameStr( layer.files[frame] );
			...
			if (layer.outPoint < compDuration) {
				...
				if (attachmentTimeline.length == 0) {
					attachmentName = this.makeEAPAttachmentName( layer, 0 );
					attachmentTimeline.push({
						"time": 0.0,
						"name": attachmentName
					});
	...
	AE2JSON.prototype.generateEAPBoneAnimations = function() {
		var boneAnimData = {};
		var layers = this.jsonData.composition.layers;
		var numLayers = layers.length;
		var numKeys, time;
			...
			if (layer.enabled) {
				...
				if (layer.transform.position.length > 1) {
					...
					for (var j=0; j<numKeys; j++) {
						...
						var tx = layer.transform.position[j][1][0] 
						var ty = -layer.transform.position[j][1][1] 
						//var sx = (layer.transform.scale_timeline[0][2][0] / 100.0);
						//var sy = (layer.transform.scale_timeline[0][2][1] / 100.0);
						var parentIndex = layer.parent;
						var parentName = bonesData[0].name;	// root
						...
						var keyData =  {
							"time": frame * frameDuration,
							"x":(tx + parentOffsetX) * this.rootScale,
							"y":(ty + parentOffsetY) * this.rootScale,
				...
				if (layer.transform.scale.length > 1) {
					...
					var sx = 1;// layer.transform.scale_timeline[0][2][0] / 100.0;
					if (sx < 0.001 && sx > -0.001) sx = sx < 0.0 ? -0.001 : 0.001;
					var sy = 1;//layer.transform.scale_timeline[0][2][1] / 100.0;
					if (sy < 0.001 && sy > -0.001) sy = sy < 0.0 ? -0.001 : 0.001;
					for (var j=0; j<numKeys; j++) {
				...
				if (layer.transform.rotation.length > 1) {
					...
					//alert(layerDataSmooth.length);
						
					if(layerDataSmooth!=null&&isSmooth){
						numKeys = layerDataSmooth[i].transform.rotation.length;
						layer=layerDataSmooth[i];
					}
					var lastValue = 0;
					...
					for (var j=0; j<numKeys; j++) {
						...
						for (var k=1; k<=steps; k++) {
							var rotation = (-layer.transform.rotation[j][1])%360;
							var keyData = {
								"time": lastTime + (dt * k),
								"test0":layer.transform.rotation_timeline[0][1],
								"test1":(-layer.transform.rotation_timeline[0][2])%360,
								"tangentType":layer.transform.rotation[j][2],
								// DEBUG STUFF
								...
								// lay chinh xac angle
								"angleExactly":rotation,
								//"test3":lastValue + (delta * k),
								//"angle": (lastValue + (delta * k)) % 360,
								"angle": lastValue + (delta * k),
								//debug
								"angleinAEP":layer.transform.rotation[j][1]
							};
							if (tangentType == "hold") {
								keyData["curve"] = "stepped";
							}
	...
	AE2JSON.prototype.generateAepData = function() {
		var layers = this.jsonData.composition.layers;
		var numLayers = layers.length;
		var bonesData = this.generateEAPBones();
		var slotsData = this.generateEAPSlots();
		var skinsData = this.generateEAPSkins();
		var boneAnimData = this.generateEAPBoneAnimations();
		var slotAnimData = this.generateEAPSlotAnimations();
		var aepData = {
			"bones": bonesData,
			"slots": slotsData,
			"skins": skinsData,
		...
		aepData["animations"][this.animationName] = {
			"bones": boneAnimData,
			"slots": slotAnimData
		}
		return aepData;
