...
namespace NO1Software.ABSystem
{
    ...
    public class AssetBundleManager : SingletonMonoBehaviour<AssetBundleManager>
    {
        ...
        {
            string depFile = string.Format("{0}/{1}", AssetBundlePathResolver.Instance.BundleCacheDir, AssetBundlePathResolver.Instance.DependFileName);
            //编辑器模式下测试AB_MODE，直接读取
//#if UNITY_EDITOR
//            depFile = AssetBundlePathResolver.Instance.GetBundleSourceFile(AssetBundlePathResolver.Instance.DependFileName, false);
//#endif
            ...
            {
                string srcURL = AssetBundlePathResolver.Instance.GetBundleSourceFile(AssetBundlePathResolver.Instance.DependFileName);
