...
namespace NO1Software.ABSystem.Editor
{
    public class ABBuilder
    {
        ...
        void InitDirs()
        {
            new DirectoryInfo(AssetBundleBuildPath.BundleSavePath).Create();
            new FileInfo(AssetBundleBuildPath.HashCacheSaveFile).Directory.Create();
        }
        ...
        {
            string path = Path.Combine(AssetBundleBuildPath.BundleSavePath, AssetBundlePathResolver.Instance.DependFileName);
        ...
        protected void RemoveUnused(List<AssetTarget> all)
        {
            ...
            DirectoryInfo di = new DirectoryInfo(AssetBundleBuildPath.BundleSavePath);
